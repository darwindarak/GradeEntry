...
var Table = React.createClass({displayName: 'Table',
    ...
    render: function() {
        ...
        var table_header = this.state.display_columns.map(function(name, i) {
            return (
                React.DOM.th( {key:i, onClick:this.sortData}, name)
                )
        }.bind(this));
        table_header.push(
            React.DOM.th( {key:"entry", onClick:this.sortData}, this.state.entry_column)
            );
        return (
            React.DOM.div( {className:"container"}, 
                React.DOM.div( {className:"row", id:"header"}, 
                    React.DOM.div( {id:"inputmask", className:"col-md-3"}, 
                        React.DOM.button( {onClick:triggerFileInput, id:"file_load", type:"button", className:"btn btn-primary"} , "Open File"),
                        React.DOM.input( {type:"file", id:"csv_source", onChange:this.loadFile})
                    ),
                    React.DOM.div( {className:"col-md-6"}, 
                        React.DOM.input( {id:"filter", className:"form-control", type:"text", placeholder:"Filter sample data", onChange:this.update_filter, onKeyUp:this.keyUp} )
                    ),
                    React.DOM.div( {className:"col-md-3"}, 
                        React.DOM.button( {id:"file_save", type:"button", className:"btn btn-success", onClick:this.saveData}, "Save File")
                    )
                ),
                React.DOM.div( {id:"data_view", className:"row"}, 
                    React.DOM.div( {className:"col-md-8 col-md-offset-2"}, 
                        React.DOM.table( {className:"table"}, 
                            React.DOM.thead(null, 
                            React.DOM.tr(null, table_header)
                            ),
                            React.DOM.tbody( {id:"dataview"}, 
                                table_rows
                            )
                        )
                    )
...
var TableRow = React.createClass({displayName: 'TableRow',
    ...
    render: function() {
        var display_columns = this.props.display_columns.map(function(colname){
            return (
                React.DOM.td( {key:colname}, this.props.data_row[colname])
            )
        }.bind(this))
        ...
        var entry_column = (
            ...
            React.DOM.td(null, 
                React.DOM.input( {key:this.props.key, type:"text", value:this.state.cell_value, onChange:this.update_cell, onKeyUp:this.update_data} )
            )
        )
